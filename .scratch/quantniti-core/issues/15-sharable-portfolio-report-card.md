...
- [ ] Report card layout is fixed-width (optimized for A4 portrait PDF and 1080×1920 social image), independent of the user's current viewport size.
- [ ] `html2canvas` and `jspdf` are loaded via CDN script tags (no npm build step required).
- [ ] Passes automated tests: report generation function produces a valid Blob output, share button triggers the expected API call or fallback behavior.
