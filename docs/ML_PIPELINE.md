...
Despite its theoretical foundation, MPT suffers from critical operational failures when applied to real-world financial markets:
...
All missing observations are handled via forward-filling followed by backward-filling to eliminate lookahead bias without introducing spurious variance.
...
Where:
* $\pi_k \ge 0, \quad \sum_{k=1}^K \pi_k = 1$ are the mixing proportions.
...
Unsupervised clustering outputs cluster labels $\{0, 1, 2\}$ with random permutations across initialization runs. To establish deterministic economic semantics, QuantNiti ranks the cluster centroids in original feature space using their **Return-to-Volatility Ratio**:
...
Sourced from audited SEBI BRSR and CRISIL disclosures:
...
3. Compute Euclidean distances and construct the linkage tree via single-linkage clustering.
...
$$
\mathbf{O} = \text{get\_quasi\_diag\_order}(\mathbf{Z}), \quad \boldsymbol{\Sigma}_{\text{diag}} = \boldsymbol{\Sigma}[\mathbf{O}, \mathbf{O}]
$$

### Stage 3: Recursive Bisection
For every sub-cluster bifurcating into sub-branches $\mathbf{C}_1$ and $\mathbf{C}_2$:
1. Calculate inverse-variance allocation weights:

$$
\tilde{w}_{k, i} = \frac{1/\sigma_i^2}{\sum_{j \in \mathbf{C}_k} 1/\sigma_j^2}, \quad \forall i \in \mathbf{C}_k
$$

2. Compute cluster variance $V_k = \tilde{\mathbf{w}}_k^T \boldsymbol{\Sigma}_k \tilde{\mathbf{w}}_k$.
3. Compute allocation split factor $\alpha = \frac{V_2}{V_1 + V_2}$.
4. Scale weights recursively: $\mathbf{w}[\mathbf{C}_1] \leftarrow \mathbf{w}[\mathbf{C}_1] \cdot \alpha$ and $\mathbf{w}[\mathbf{C}_2] \leftarrow \mathbf{w}[\mathbf{C}_2] \cdot (1 - \alpha)$.

---

### 6.4 Risk Persona & ESG Allocation Constraints

| Risk Persona | Max Single Stock Weight | Min Asset Diversification | Sector Cap | ESG Tilting Formula |
| :--- | :--- | :--- | :--- | :--- |
...
Retail brokerage APIs (Zerodha Kite, Groww, Upstox) strictly reject fractional shares. QuantNiti implements a **Greedy Whole-Share Sizing Algorithm**:

### 7.1 Mathematical Sizing
Given total investable capital $C$ and continuous target weights $w_i^*$:

$$
n_i = \left\lfloor \frac{w_i^* \cdot C}{P_i} \right\rfloor, \quad C_{\text{spent}} = \sum_{i=1}^N n_i \cdot P_i
$$
...
Residual cash is reallocated iteratively to the highest-conviction underweight assets without violating risk bounds:
...
For monthly deposits $P_{\text{monthly}}$ with an annual step-up percentage $s$:

$$
P_{m} = P_{\text{monthly}} \cdot (1 + s)^{\lfloor (m-1)/12 \rfloor}, \quad V_M = \sum_{m=1}^M P_m \left(1 + \frac{r_{\text{cagr}}}{12}\right)^{M - m + 1}
$$

### 8.3 The Compounding Tipping Point
The month $t^*$ where **monthly compounding interest earnings exceed the monthly out-of-pocket deposit**:
...
## 10. End-to-End Execution Flowchart & Mathematical Reference
