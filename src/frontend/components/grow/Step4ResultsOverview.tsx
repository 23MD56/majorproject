...
export const Step4ResultsOverview: React.FC<Step4ResultsOverviewProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <GlassCard className="p-4 sm:p-6 bg-violet-950/20 border-violet-500/20 flex flex-col items-center">
        <AllocationDonut allocations={allocations} totalInvested={totalInvested} />

        {/* Discrete Allocation & Cash Buffer Summary */}
        <div className="w-full grid grid-cols-2 gap-3 mt-4 pt-4 border-t border-violet-500/20">
          <div className="p-3 rounded-xl bg-slate-900/60 border border-violet-500/20 text-center">
          ...
          {allocations.map((item, index) => {
            ...
            return (
              <div
                ...
                className="flex items-center justify-between p-3 rounded-xl bg-slate-900/60 border border-violet-500/20 hover:border-violet-500/40 transition-colors"
              >
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-lg bg-violet-600/20 border border-violet-500/30 flex items-center justify-center text-xs font-bold text-violet-300">
                    {item.symbol?.substring(0, 2)}
                  </div>
                  <div>
                    <p className="text-xs sm:text-sm font-bold text-white">{item.symbol}</p>
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between">
          ...
          <span className="text-[10px] text-slate-400 font-medium">Quantile Regression (ML)</span>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          {/* Base Q50 */}
          <div className="p-3 rounded-xl bg-violet-950/40 border border-violet-500/40 text-center relative overflow-hidden">
            <div className="absolute top-0 right-0 w-8 h-8 bg-violet-500/10 rounded-bl-xl pointer-events-none" />
            <span className="text-[10px] font-semibold text-violet-300 uppercase tracking-wider block">
              Base Case (Q50)
            </span>
            <p className="text-xs sm:text-sm font-bold text-white mt-1">
